...
###################################
# install python3-smbus runtime #
###################################
	echo -e "\n    Installing  python3-smbus \n"

	if sudo apt-get install python3-smbus -y;then
		echo -e "    Successfully installed python3-smbus \n"
		is_installed_python_smbus=true
	else
		echo -e "    Failed to installed python3-smbus \n"
		echo -e "    Do you want to skip this? \c"
		if_continue
		if [ $? = 1 ] ; then
			echo -e "    Skipped python3-smbus installation."
		else
			end
		fi
	fi
...
###################################
	...
	echo -e "Installing PyPi Modules \n"
